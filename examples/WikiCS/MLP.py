...
class MLP(nn.Module):
    ...
    def __init__(self, num_in, num_hid, num_out, mathcal_Z, dropout, device):
        ...
        # Linear layers
        self.NN_1 = nn.Linear(num_in, num_hid)
        self.NN_2 = nn.Linear(num_hid, num_hid)
        self.NN_3 = nn.Linear(num_hid, num_out)
    ...
    def forward(self, x):
        """
            Forward module of MLP
                ...
                Returns:
                    z : torch.tensor of shape (num_examples, num_classes)
                        Representations learnt by the final layer DNN
